...
public class SimpleCluster extends ArrayList<Pixel> {
	...
	List<Pixel> members = getNeighbouringPoints(points, distances,
		meanDistance);
     ...
     * 
     * @param pixels
     *            the pixels to select from
     * @param distances
     *            array of distances whose index corresponds to the pixels
     *            position in the pixels list
     * @param d
     *            max distance from centroid
     * @return list of matching points
     */
    private static List<Pixel> getNeighbouringPoints(List<Pixel> pixels,
	    double[] distances, double d) {
     ...
     * @param points
     * @param distances
     *            this array is filled with every points distance to the
     *            centroid using its position in the points list
    ...
    private static double calculateDistances(Pixel centroid, ArrayList<Pixel> points, double[] distances) {
		...
		//TODO do something to show that the return values are correct 
		//got a nullpointer when no clusters were available.
		try {
		    return distanceSum/points.size();
		} catch (ArithmeticException ae) {
		    Log.e(Constants.LOG_TAG, ae.getMessage());
		    ae.printStackTrace();
		}
		return 0;
	}
