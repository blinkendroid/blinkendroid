...
public class PlayerThread extends Thread {
    ...
    private PlayerProtocolHandler protocolHandler;
    private BLM blm;
	...
	Log.i(Constants.LOG_TAG, "setGlobalTime(" + globalTime + ") diff "
		+ (this.globalTime - globalTime));
	this.globalTime = globalTime;
    }

    public void restart(long globalTime) {
	Log.i(Constants.LOG_TAG, "restart(" + globalTime + ") diff "
		+ (this.globalTime - globalTime));
	restart = true;
	this.globalTime = globalTime;
    }
	...
	while (playing) {
		...
		if (!jumpframe) {
		    playerView.setMatrix(clipMaptrix(frame.matrix, blm.width,
			    blm.height), blm.width, blm.height, i);
		    playerView.postInvalidate();
		} else {
		    Log.i("PlayerThread", "jump frame " + i
			    + " cause waitTime " + waitTime);
		}
		i++;
	    ...
	    // Log.i("PlayerThread",
	    // "played movie in "+(System.currentTimeMillis()-startTime));
	}
    }
