...
public class PlayerManager implements ConnectionListener, CommandHandler {
	...
	*/
	...
	public synchronized PlayerClient addClientToMatrix(PlayerClient playerClient) {
		...
		if (!found) {
			...
			clip(false);
			playerClient.getBlinkenProtocol().clip(playerClient.startX, playerClient.startY, playerClient.endX, playerClient.endY);
		}
	...
	public synchronized void removeClientFromMatrix(PlayerClient playerClient) {
		...
		logger.info("removeClient " + playerClient);
	...
	public void switchMovie(BLMHeader blmHeader) {
		if (videoServer == null) {
			...
			for (int i = 0; i < maxY; i++) {
				for (int j = 0; j < maxX; j++) {
					if (null != mMatrixClients[i][j]) {
						logger.info("play PlayerClient " + mMatrixClients[i][j] + " " + filename);
						mMatrixClients[i][j].getBlinkenProtocol().play(startTime, BlinkendroidProtocol.OPTION_PLAY_TYPE_MOVIE);
					}
				}
			}
			clip(true);
		...
		if (videoServer == null) {
			...
			for (int i = 0; i < maxY; i++) {
				for (int j = 0; j < maxX; j++) {
					if (null != mMatrixClients[i][j]) {
						logger.info("play PlayerClient " + mMatrixClients[i][j] + " " + filename);
						mMatrixClients[i][j].getBlinkenProtocol().play(startTime, BlinkendroidProtocol.OPTION_PLAY_TYPE_IMAGE);
					}
				}
			}
			clip(true);
	...
	public void handle(UDPDirectConnection blinkendroidprotocol, InetSocketAddress socketAddr, int proto, ByteBuffer protoData) {
	...
	*/
	...
	public void locateMe(PlayerClient playerClient) {
		logger.info("locateMe from " + playerClient);
		arrow(playerClient);
	}
